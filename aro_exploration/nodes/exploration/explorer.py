...
class Explorer:
    def __init__(self):
        ...
        # Using frontier.py
        self.get_closest_frontier = rospy.ServiceProxy(
            "get_closest_frontier",
            GenerateFrontier,
        )
        self.wait_for_service(self.get_closest_frontier)
        self.get_best_value_frontier = rospy.ServiceProxy(
        ...
        self.marker_sub = rospy.Subscriber(
            ...
            PoseStamped,
            self.relative_marker_pose_cb,
        )
        self.follow_path = actionlib.SimpleActionClient("follow_path", FollowPathAction)
        while (
            not self.follow_path.wait_for_server(timeout=rospy.Duration(1.0))
    ...
    def lookup_transform(
        ...
        timeout=rospy.Duration.from_sec(0.0),
    ) -> TransformStamped:
    ...
    def relative_marker_pose_cb(self, msg: PoseStamped):
        ...
        rospy.loginfo(
            "Marker localized at pose: [%.1f,%.1f].",
    ...
    def get_feasible_goal(self) -> Tuple[Optional[Pose2D], Optional[List[Pose2D]]]:
        ...
        #     path = []
        path_request = PlanPathRequest(self.get_robot_pose(), goal)
        path_response = self.plan_path(path_request)
        path = path_response.path
        # if not path or self.marker_pose is not None:
        #     return goal, []
        # goal_idx = 4 * len(path) // 5
        # goal = path[goal_idx]
        path: Optional[List[Pose2D]] = path  # path[:goal_idx]
    ...
    def update_plan(self, event):
        try:
            with self.lock:
                ...
                if (
                    self.marker_pose is not None
                    and np.linalg.norm(pose - array(self.init_pose)) <= 0.25
                ):
                    finished = True
                else:
                    finished = False
                ...
                if self.current_goal is None:
                    rospy.logwarn(
                        "Feasible goal not found during %i retries.",
                        self.retries,
                    )
                    return
                ...
                self.follow_path.send_goal(
                    follow_path_goal,
                    feedback_cb=self.follow_path_feedback_cb,
    ...
    def follow_path_feedback_cb(self, feedback: FollowPathFeedback):
        rospy.loginfo(
            "Received control feedback, tracking error is: %.2f.",
            feedback.error,
        )
